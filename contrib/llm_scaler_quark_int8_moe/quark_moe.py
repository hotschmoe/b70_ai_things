...
# --- b70 patch: detect a dynamic-per-token INT8 W8A8 MoE (per-channel weights). ---
# The stock image only wires Quark MoE for fp8-w4a8 / fp8-w8a8 / ocp-mx and raises
# "Unsupported FusedMoe scheme" for int8. The existing QuarkConfig._is_static_tensor_w8a8
# only matches STATIC per-tensor int8 (linear path); this checkpoint is DYNAMIC per-token
# int8 activations + per-channel symmetric int8 weights, so we detect that here and route
# it to QuarkW8A8Int8MoEMethod (mirrors CompressedTensorsW8A8Int8MoEMethod, which already
# ships in this image and is the same vLLM version's int8 fused-MoE kernel).
def _is_quark_w8a8_int8_dynamic_moe(weight_quant, input_quant):
...
class QuarkMoEMethod(FusedMoEMethodBase):
    ...
    def get_moe_method(
        ...
        elif _is_quark_w8a8_int8_dynamic_moe(weight_config, input_config):
            # b70 patch: int8 W8A8 fused MoE (dynamic per-token act, per-channel weights).
...
# --- b70 patch: INT8 W8A8 fused-MoE method for Quark checkpoints. -----------------
# Mirrors CompressedTensorsW8A8Int8MoEMethod (same image / same vLLM version) but uses
# Quark's dict-based config + 1-dim per-channel scale layout (matching QuarkW8A8Fp8
# MoEMethod's per_channel path: load a 2-D scale, then unsqueeze to (E, out, 1) for the
# int8 fused-MoE kernel). Dynamic per-token int8 activations => no input scales.
class QuarkW8A8Int8MoEMethod(QuarkMoEMethod):
    def __init__(
        ...
        if self.weight_qscheme != "per_channel":
            raise ValueError(
                "Quark INT8 Fused MoE only supports per-channel weight "
                f"quantization. Found weight qscheme={self.weight_qscheme}."
            )
        self.static_input_scales = not self.input_quant.get("is_dynamic")
        if self.static_input_scales:
            raise ValueError(
                "Quark INT8 Fused MoE requires dynamic per-token activation "
                "quantization (input_tensors is_dynamic=True)."
            )
    ...
    def create_weights(
        ...
        **extra_weight_attrs,
        ...
        # PER-CHANNEL weight scales. Quark stores 1-dim scales -> 2-D param per layer
        # (E, out); process_weights_after_loading unsqueezes to (E, out, 1).
        w13_weight_scale = torch.nn.Parameter(
            torch.ones(
        ...
        # Dynamic per-token int8 activations -> no precomputed input scales.
        layer.w13_input_scale = None
        layer.w2_input_scale = None

    def process_weights_after_loading(self, layer: torch.nn.Module) -> None:
        # int8 fused-MoE kernel expects (E, out, 1) channel scales.
