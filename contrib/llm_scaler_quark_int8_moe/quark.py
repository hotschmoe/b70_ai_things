...
# b70 patch: parameter classes + helper for the int8 dequant linear scheme below.
...
# --- b70 patch: dynamic-per-token INT8 W8A8 detector (per-channel symmetric weights). ---
# Mirrors _is_quark_w8a8_int8_dynamic_moe in quark_moe.py. The stock _is_static_tensor_w8a8
# only matches STATIC per-tensor int8; this checkpoint (nameistoken Qwen3.6-35B-A3B-Quark-
# W8A8-INT8) is DYNAMIC per-token int8 activations + per-channel symmetric int8 weights.
def _is_quark_w8a8_int8_dynamic_linear(weight_quant, input_quant):
    if weight_quant is None or input_quant is None:
        return False
    is_int8 = (
        weight_quant.get("dtype") == "int8" and input_quant.get("dtype") == "int8"
    )
    w_per_channel = weight_quant.get("qscheme") == "per_channel"
    w_static_symmetric = (
        not weight_quant.get("is_dynamic")
    ) and weight_quant.get("symmetric") is True
    a_dynamic = bool(input_quant.get("is_dynamic"))
    return is_int8 and w_per_channel and w_static_symmetric and a_dynamic


class QuarkW8A8Int8DequantXPU(QuarkScheme):
    """b70 patch: weight-only int8 -> bf16 dequant linear for XPU.

    The 0.14.1 image has NO XPU int8 scaled-mm kernel (_POSSIBLE_KERNELS has only
    CPU/CUDA/ROCM -> choose_scaled_mm_linear_kernel raises on XPU), so the stock
    QuarkW8A8Int8 linear scheme cannot run here. This dequantizes the per-channel
    symmetric int8 weights to the model dtype once at load and runs a plain GEMM;
    activations are NOT quantized (effectively W8A16 -- more accurate than the
    checkpoint's W8A8, correctness-first). Used only for the (minority) linear
    layers (linear_attn projections + shared_expert); the 256 routed experts stay
    true int8 via quark_moe.QuarkW8A8Int8MoEMethod.
    """

    def __init__(self, qscheme, input_symmetric):
    ...
    def create_weights(
        ...
        **kwargs,
        ...
        # per-channel symmetric -> only a weight_scale (no zero_point in this ckpt).
    ...
    def process_weights_after_loading(self, layer: torch.nn.Module) -> None:
        w = layer.weight  # int8 [N, K]
        # Compute OUTSIDE inference_mode so w_deq is a normal tensor with a version
        # counter -- an inference tensor here breaks torch.compile functionalization
        # ("Inference tensors do not track version counter") when F.linear is traced.
        with torch.inference_mode(False), torch.no_grad():
            ws = layer.weight_scale.to(torch.float32).reshape(-1, 1)  # [N, 1]
...
class QuarkConfig(QuantizationConfig):
    ...
    def _get_scheme_from_config(self, config: dict[str, Any]) -> "QuarkScheme":
        ...
        elif _is_quark_w8a8_int8_dynamic_linear(weight_config, input_config):
            # b70 patch: dynamic-per-token int8 linear -> bf16 dequant GEMM (no XPU
            # int8 scaled-mm kernel in this image).
