...
> A bounded XPU profile now shows 41 fence resets, 41 host event waits, and 82
> command-list submissions per token, exactly matching the 41 graph pieces.
...
> binary provenance; June record's 67 MB `_xpu_C` remains to reconstruct -> DONE
> raw oneCCL direct-P2P plus XPUGraph oracle on this host. The supplied LocalMaxxing row is
...
> does not explain our 17.06. The exact `[4,5120]` direct-plus-XPUGraph oneCCL
...
> ### [CAMPAIGN 2026-08-24] -- C1 GO, C2 NO-GO, C3a PROMOTED, C3b integration NO-GO
...
> current push at 10/110/440 KiB, so NO-GO. C3a native target/MTP embedding
...
> speed claim. The fused primitive is now exact and 1.78-1.93x at measured
> M<=11 rows (M9 safely excluded), and its strict 131K mechanism gate passed
> 40960/40960 with generic=0 on both ranks. It is not promoted: the first
> serving pair won fixed-content soaks by 11-12% but lost mandatory phase and
...
> GPTQ GRAPH=1 TP=1 c1 **25.0**. TP=2 GRAPH=0 PUSH_AR load-gate c1 3.7.
...
> LOOP 104: dual 1-card. Ornith MixedCal-v2 `:18080` MTP1 131k KV 151k c1 66.1; 3.6 NVFP4 `:18081` 100k KV 144k c1 64.5. W4A8 TP=2 262k trial stopped.
...
>    visible `!` garbage in 1/18 streams. Replicating the 682 MiB packed drafter head then removed
>    every token-selection collective at 341 MiB extra/rank, but c1 fell to 30.1 and rank 1 produced
>    9/64 intermittent mismatch records even though both ranks' full weight/scale SHA256 values
>    exactly matched the checkpoint. The local-top1 and replicated-head branches are closed and
>    default-off. Capturing the eager gather is the only remaining communication-first route, and
...
>    /mnt/vm_8tb/b70/zml-intel; #706 is oneDNN FP8 scaled matmul, not INT8).
>    Journal + commit at each milestone.
...
>    host-PID GRAPH=1 G1 (HOSTNS hang). S2b
>    GRAPH=0 TP=2 c1 13.4. S2c 0.963/0.915.
