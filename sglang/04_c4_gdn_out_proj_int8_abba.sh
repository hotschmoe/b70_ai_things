#!/usr/bin/env bash
# Position-balanced qualification for GDN out_proj-only INT8.
#
...
# B = GDN out_proj-only INT8 checkpoint plus the generated corrected config overlay.
...
PREPARE="$REPO/sglang/prepare_c4_gdn_out_proj_candidate.py"
ANALYZER="$REPO/sglang/analyze_c4_gdn_out_proj_int8_abba.py"
SELF="$REPO/sglang/04_c4_gdn_out_proj_int8_abba.sh"
STAMP="${STAMP:-$(date -u +%Y%m%dT%H%M%SZ)}"
OUT="${OUT:-$REPO/results/logs/c4_gdn_out_proj_int8_abba_$STAMP}"
PORT="${PORT:-31003}"
CTX="${CTX:-131072}"
MAXREQ="${MAXREQ:-4}"
MODEL_A_CONTAINER="/models/qwen3.6-27b/w8a8-sqgptq"
MODEL_B_CONTAINER="/models/qwen3.6-27b/w8a8-sqgptq-gdn-out-proj-int8"
MODEL_A_HOST="$REPO/models/files/qwen3.6-27b/w8a8-sqgptq"
MODEL_B_HOST="$REPO/models/files/qwen3.6-27b/w8a8-sqgptq-gdn-out-proj-int8"
SERVED_A="qwen36-27b-W8A8-sqgptq-base-mtp-c4-abba"
SERVED_B="qwen36-27b-W8A8-sqgptq-GDN-OUT-RTN-mtp-c4-abba"
...
say() { echo "[c4-gdn-out-abba $(date -u +%H:%M:%S)] $*"; }
...
artifacts=(
  ...
  "$PREPARE"
  "$ANALYZER"
  "$REPO/sglang/configs/qwen36_w8a8_sqgptq_gdn_out_proj_rtn_quantization.json"
  ...
  "$MODEL_B_HOST/GDN_OUT_PROJ_INT8_NOTE.txt"
)
...
IMAGE_ID="$(docker image inspect sglang-xpu:mtp --format '{{.Id}}')"
{
  ...
  echo "claim=c4_gdn_out_proj_int8_abba_qualification"
  ...
  echo "order=01_A1:base,02_B1:out-proj-int8,03_B2:out-proj-int8,04_A2:base"
