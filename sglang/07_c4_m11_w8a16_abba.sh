#!/usr/bin/env bash
# Position-balanced qualification for M<=11 W8A16 routing.
# Run through the external dual-card lease:
#   ./bin/gpu-run bash sglang/07_c4_m11_w8a16_abba.sh
# A = W8A16 M max 1. B = W8A16 M max 11. All other state is identical.
...
ANALYZER="$REPO/sglang/analyze_c4_m11_w8a16_abba.py"
SELF="$REPO/sglang/07_c4_m11_w8a16_abba.sh"
STAMP="${STAMP:-$(date -u +%Y%m%dT%H%M%SZ)}"
OUT="${OUT:-$REPO/results/logs/c4_m11_w8a16_abba_$STAMP}"
PORT="${PORT:-31007}"
CTX="${CTX:-131072}"
MAXREQ="${MAXREQ:-4}"
MODEL_CONTAINER="/models/qwen3.6-27b/w8a8-sqgptq"
MODEL_HOST="$REPO/models/files/qwen3.6-27b/w8a8-sqgptq"
SERVED_A="qwen36-27b-W8A8-sqgptq-mtp-mmax1-c4-abba"
SERVED_B="qwen36-27b-W8A8-sqgptq-mtp-mmax11-c4-abba"
...
say() { echo "[c4-m11-abba $(date -u +%H:%M:%S)] $*"; }
...
require_external_dual_card_lease() {
  ...
  flock -n 8 || { echo "card-0 lease is not locked" >&2; return 2; }
  flock -n 9 || { echo "card-1 lease is not locked" >&2; return 2; }
  {
...
ensure_endpoints_down() {
  ...
  if [ -n "$active_name" ] && docker ps --filter "name=^/${active_name}$" \
    --format '{{.Names}}' | rg -qx "$active_name"; then failed=1; fi
  if docker ps --filter "name=^/${PROD_NAME}$" --format '{{.Names}}' \
    | rg -qx "$PROD_NAME"; then failed=1; fi
  if curl -fsS --max-time 3 "http://127.0.0.1:$PORT/health" \
    >/dev/null 2>&1; then failed=1; fi
...
IMAGE_ID="$(docker image inspect sglang-xpu:mtp --format '{{.Id}}')"
{
  ...
  echo "order=01_A1:mmax1,02_B1:mmax11,03_B2:mmax11,04_A2:mmax1"
...
if docker ps --filter "name=^/${PROD_NAME}$" --format '{{.Names}}' \
  ...
  >/dev/null 2>&1; then
  echo "unknown endpoint on production port; refusing" >&2
  exit 2
fi
if curl -fsS --max-time 3 "http://127.0.0.1:$PORT/health" \
  >/dev/null 2>&1; then
  echo "experiment port is already serving; refusing" >&2
  exit 2
fi
"$REPO/bin/xpu-health" 2>&1 | tee "$OUT/health_pre_campaign.log"

run_fixed_output() {
  local served="$1" output="$2"
...
run_arm() {
  local label="$1" variant="$2" threshold served slug name arm_dir
  if [ "$variant" = A ]; then threshold=1; served="$SERVED_A"; else threshold=11; served="$SERVED_B"; fi
  slug="$(tr '[:upper:]' '[:lower:]' <<<"$label")"
  name="c4_m11_abba_$slug"
  arm_dir="$OUT/$label"
  mkdir -p "$arm_dir"
  active_name="$name"
  active_dir="$arm_dir"

  say "ARM $label start variant=$variant m_max=$threshold served=$served"
  ...
  CTX="$CTX" RADIX=0 MAXREQ="$MAXREQ" PORT="$PORT" NAME="$name" \
    SERVED="$served" IMG=sglang-xpu:mtp CKPT="$MODEL_CONTAINER" API_KEY= \
    CONFIG_OVERLAY= MEMFRAC=0.90 TOOLCALL=1 TOOLPARSER=qwen3_coder \
    REASONPARSER=qwen3 METRICS=1 THINKCAP=4096 KDIR="$KDIR" \
    PUSHDIR="$PUSHDIR" PUSH_AR_SO="$PUSH_AR_SO" SPEC_STEPS=10 \
    SPEC_DRAFT=11 REPLICATE_MTP_EMBED=1 PUSH_AR=1 PUSH_AR_MIN_NUMEL=0 \
    PUSH_AR_MAXB="$PUSH_AR_MAXB" DELAY_MLP_AR=0 FUSED_MLP_AR_NORM=0 \
    LMHEAD_INT8=0 W8A16_M_MAX="$threshold" W8A16_ROUTE_DEBUG=0 \
    bash "$SHELF" start 2>&1 | tee "$arm_dir/start.log"

  curl -fsS --max-time 15 "http://127.0.0.1:$PORT/v1/models" >"$arm_dir/models.json"
  curl -fsS --max-time 30 "http://127.0.0.1:$PORT/get_server_info" >"$arm_dir/server_info.json"
  docker inspect "$name" >"$arm_dir/container_inspect.json"

  run_fixed_output "$served" "$arm_dir/fixed_output.json" 2>&1 | tee "$arm_dir/fixed_output.log"
