#!/usr/bin/env bash
# serve_35b_w8a8_moe.sh -- RESEARCH serve (NOT a shelf entry yet) for the Qwen3.6-35B-A3B Quark W8A8
# INT8 MoE on sglang-XPU (Route A: in-tree Triton fused_moe use_int8_w8a8). Gated on the Route-A probe
# (research/w8a8/sglang_moe_int8_probe.py). Promote to rdy_to_serve/sglang/qwen36-35b-a3b-w8a8 ONLY
# after it passes the coherence gate + a soak (shelf rule: measured coherent before it lands).
#
# WIRING (mount-not-bake, same model as the 27B w8a8 entry):
#   - IMG sglang-xpu:mtp (ships the in-tree int8 fused_moe Triton kernel; see SGLANG_MOE_PLAN.md sec 1)
#   - mount the updated woq_shim.py (adds the B70_QUARK_MOE_INT8 auto-install hook) + quark_moe_int8.py
#     into site-packages; B70_QUARK_MOE_INT8=1 makes woq_shim call quark_moe_int8.install() in EVERY
#     process (the model builds in the TP workers, so a main-process-only wrapper would miss them).
#   - 256 routed experts -> TRUE int8 (Triton use_int8_w8a8); dense linears (linear_attn.*,
#     shared_expert.*) -> int8->bf16 dequant (DEFAULT, correctness-first; XPU has no int8 scaled-mm).
#   - GDN-safe (carry the 27B settings): --mamba-ssm-dtype float32, --skip-server-warmup, triton
#     linear-attn, --disable-cuda-graph. NO MTP (get the base MoE int8 serving first).
#
#   /mnt/vm_8tb/b70/gpu-run bash serve_35b_w8a8_moe.sh start   # serve TP=2, coherence-gated, stay up
#   bash serve_35b_w8a8_moe.sh bench                            # c1+c4 regime bench (perf_regime.sh)
#   bash serve_35b_w8a8_moe.sh stop                             # stop + release, health re-probe
#   /mnt/vm_8tb/b70/gpu-run bash serve_35b_w8a8_moe.sh run      # start + bench + stop in one lease
set -uo pipefail
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
ROOT="${ROOT:-/mnt/vm_8tb/b70}"; REPO="${REPO:-/mnt/vm_8tb/github/b70_ai_things}"
IMG="${IMG:-sglang-xpu:mtp}"
NAME="${NAME:-sglang_w8a8_moe_35b}"
CKPT="${CKPT:-/models/qwen3.6-35b-a3b/quark-w8a8-int8}"
TOK="${TOK:-/models/qwen3.6-35b-a3b/quark-w8a8-int8}"   # the quark ckpt ships its own tokenizer.json
SERVED="${SERVED:-qwen36-35b-a3b-quark-w8a8-int8}"
PORT="${PORT:-30000}"; TP="${TP:-2}"; CTX="${CTX:-8192}"; MEMFRAC="${MEMFRAC:-0.88}"
MAXREQ="${MAXREQ:-8}"
SHIM="$REPO/sglang/images/sglang-xpu-mtp/woq_shim.py"      # the woq_shim WITH the B70_QUARK_MOE_INT8 hook
LOADER="$REPO/sglang/patches/quark_moe_int8.py"
say(){ echo "[$(date +%H:%M:%S)] $*"; }
SP=/opt/venv/lib/python3.12/site-packages

start(){
  say "pre-flight xpu-health"; "$REPO/bin/xpu-health" 2>&1 | tail -2 || { say "UNHEALTHY -- abort"; return 3; }
  [ -f "$LOADER" ] || { say "missing loader: $LOADER"; return 2; }
  [ -f "$SHIM" ] || { say "missing woq_shim: $SHIM"; return 2; }
  docker rm -f "$NAME" >/dev/null 2>&1
  say "serve W8A8 int8 MoE (Route A, dequant dense) TP=$TP -> $SERVED on :$PORT (img=$IMG)"
  docker run -d --name "$NAME" --device /dev/dri -v /dev/dri/by-path:/dev/dri/by-path \
    --ipc=host --shm-size 32g -p "${PORT}:${PORT}" \
    -v "$REPO/models/files:/models:ro" \
    -v "$ROOT/hf_cache:/hf_cache" -v "$ROOT/sgl_cache:/sgl_cache" \
    -v "$LOADER:$SP/quark_moe_int8.py:ro" \
    -v "$REPO/sglang/patches/int8_actquant_xpu.py:$SP/int8_actquant_xpu.py:ro" \
    -v "$SHIM:$SP/woq_shim.py:ro" \
    -e HF_HOME=/hf_cache -e XDG_CACHE_HOME=/sgl_cache -e TORCHINDUCTOR_CACHE_DIR=/sgl_cache/inductor \
    -e TRITON_CACHE_DIR=/sgl_cache/triton \
    -e B70_QUARK_MOE_INT8=1 -e B70_MOE_DEBUG="${B70_MOE_DEBUG:-0}" \
    "$IMG" bash -c "source /opt/intel/oneapi/setvars.sh --force >/dev/null 2>&1; \
      export LD_LIBRARY_PATH=/opt/intel/oneapi/compiler/2025.3/lib:\$LD_LIBRARY_PATH; \
      exec python -m sglang.launch_server --model-path '$CKPT' --tokenizer-path '$TOK' \
      --served-model-name '$SERVED' --trust-remote-code \
      --device xpu --attention-backend intel_xpu --linear-attn-backend triton \
      --disable-cuda-graph --mamba-ssm-dtype float32 --disable-overlap-schedule \
      --page-size 64 --disable-radix-cache --skip-server-warmup \
      --tp $TP --context-length $CTX --mem-fraction-static $MEMFRAC --max-running-requests $MAXREQ \
      --host 0.0.0.0 --port $PORT" >/dev/null
  say "waiting for /health (load ~2-4min; first run JIT-compiles the int8 MoE Triton kernel)..."
  for i in $(seq 1 160); do
    docker ps --filter "name=$NAME" --format '{{.Names}}' | grep -q "$NAME" || { say "EXITED"; docker logs "$NAME" 2>&1|tail -50; return 1; }
    [ "$(curl -s -o /dev/null -w '%{http_code}' http://localhost:$PORT/health 2>/dev/null||echo 000)" = 200 ] && { say "/health 200 (~$((i*5))s)"; break; }
    sleep 5
  done
  [ "$(curl -s -o /dev/null -w '%{http_code}' http://localhost:$PORT/health 2>/dev/null||echo 000)" = 200 ] || { say "NEVER HEALTHY"; docker logs "$NAME" 2>&1|tail -50; return 1; }
  local g; g=$(curl -s --max-time 180 "http://localhost:$PORT/v1/chat/completions" -H 'content-type: application/json' \
    -d "{\"model\":\"$SERVED\",\"messages\":[{\"role\":\"user\",\"content\":\"Why is the sky blue? Two sentences.\"}],\"max_tokens\":64,\"temperature\":0}")
  echo "$g" | python3 -c "import sys,json
c=json.load(sys.stdin)['choices'][0]['message']['content'] or ''
print('COHERENCE OK:',repr(c[:160])) if c.strip() and (len(c)<16 or max(c.count(x) for x in set(c))/len(c)<0.6) else (print('GATE FAIL:',repr(c[:120])) or sys.exit(1))" \
    || { say "coherence gate FAILED -- see: docker logs $NAME"; return 1; }
  say "healthy + coherent; serving $SERVED on :$PORT"
}
stop(){ docker rm -f "$NAME" >/dev/null 2>&1; say "stopped $NAME"; "$REPO/bin/xpu-health" 2>&1 | tail -2 || true; }
bench(){ bash "$REPO/sglang/perf_regime.sh" "$NAME" "$PORT" "$SERVED" "$TOK" "w8a8-moe-int8"; }
gen(){ curl -s "http://localhost:$PORT/v1/chat/completions" -H 'content-type: application/json' \
        -d "{\"model\":\"$SERVED\",\"messages\":[{\"role\":\"user\",\"content\":\"${2:-Why is the sky blue?}\"}],\"max_tokens\":128,\"temperature\":0}"; }

case "${1:-start}" in
  start) start ;;
  stop)  stop ;;
  bench) bench ;;
  gen)   gen "$@" ;;
  run)   start && bench; rc=$?; stop; exit $rc ;;
  smoke) start; rc=$?; stop; exit $rc ;;
  *) echo "usage: serve_35b_w8a8_moe.sh {start|stop|bench|gen|run|smoke}"; exit 2 ;;
esac
