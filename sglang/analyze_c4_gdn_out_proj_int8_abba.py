#!/usr/bin/env python3
"""Parse and fail-closed gate the GDN out_proj-only INT8 A-B-B-A campaign."""
...
def gm(left: float, right: float) -> float:
    return math.sqrt(left * right)


def spread(left: float, right: float) -> float:
    mean = (left + right) / 2.0
...
def inspect_config(
    path: Path, arm: str, manifest: dict[str, str], repo: Path
) -> bool:
    data = load_json(path)
    ...
    expected_model = manifest["model_b" if candidate else "model_a"]
    expected_served = manifest["served_b" if candidate else "served_a"]
    ...
    def mounted(destination: str, source: str) -> bool:
        return any(
    ...
    if candidate:
        config_ok = (
            len(config_mounts) == 1
            and mounted(f"{expected_model}/config.json", manifest["overlay"])
        )
    else:
        config_ok = len(config_mounts) == 0

    command = " ".join(str(part) for part in item.get("Config", {}).get("Cmd", []))
    command_ok = (
        f"--model-path '{expected_model}'" in command
        and f"--served-model-name '{expected_served}'" in command
        and "--tp 2" in command
    ...
    return (
        ...
        and mounted(
            ...
            str(repo / "sglang/patches/w8a8_shim.py"),
        )
        and mounted(
            "/opt/venv/lib/python3.12/site-packages/mtp_replicated_embedding.py",
            str(repo / "sglang/patches/mtp_replicated_embedding.py"),
        )
        and mounted(
            "/opt/venv/lib/python3.12/site-packages/push_ar_xpu.py",
            str(repo / "sglang/patches/push_ar_xpu.py"),
        )
    )


def server_info_exact(info: dict, arm: str, manifest: dict[str, str]) -> bool:
    candidate = arm in B_ARMS
    return (
        info.get("status") == "ready"
        and info.get("model_path") == manifest["model_b" if candidate else "model_a"]
        and info.get("served_model_name")
        == manifest["served_b" if candidate else "served_a"]
        and int(info.get("context_length") or 0) == int(manifest["ctx"])
        and int(info.get("tp_size") or 0) == 2
        and int(info.get("pp_size") or 0) == 1
        and int(info.get("max_running_requests") or 0)
        == int(manifest["maxreq"])
...
def parse_arm(root: Path, arm: str, manifest: dict[str, str]) -> dict:
    directory = root / arm
    ...
    return {
        ...
        "phase_cv_pct": cv(
            [float(row["post_first_tok_s"]) for row in phase_runs]
        ),
        "perf_c1_decode": value(r"WARM\[c1\] decode=([0-9.]+)", perf, "perf c1"),
        "perf_c1_ttft_ms": value(
            r"WARM\[c1\].*TTFT=([0-9.]+)ms", perf, "perf c1 TTFT"
        ),
        "perf_c4_stream": value(
            r"WARM\[c4\] decode=([0-9.]+)", perf, "perf c4"
        ),
        "perf_c4_agg": value(
            r"WARM\[c4\].*agg_out=([0-9.]+)", perf, "perf c4 agg"
        ),
        ...
        "soak_ratio": value(
            r"first/last window ratio ([0-9.]+)x", soak, "soak ratio"
        ),
        "prefill_c1_ttft_ms": value(
            r"TTFT avg=([0-9.]+)ms", pre1, "prefill c1"
        ),
        "prefill_c4_ttft_ms": value(
            r"TTFT avg=([0-9.]+)ms", pre4, "prefill c4"
        ),
        ...
        and "=== 24 streams:" in mixed,
        "soak_coherent": "coherence OK" in soak,
        "fatal": FATAL_RE.search(server) is not None,
        "w8a8_installed": server.count("[w8a8-shim] installed: FUSED hybrid") >= 2,
        ...
        "forbidden_feature_marker": any(
            marker in server
            for marker in ("[lmhead-int8]", "[delayed-mlp-ar] ENABLED", "[fused-mlp-ar-norm]")
        ),
    }


def make_performance_checks(rows: dict[str, dict], deltas: dict[str, float]) -> dict[str, bool]:
    latency_keys = (
        "phase_ttft_ms",
        "perf_c1_ttft_ms",
        "perf_c4_ttft_ms",
...
def main() -> int:
    ...
    deltas = {
        f"{key}_pct": throughput_delta(rows, key) for key in throughput_keys
    }
    deltas.update(
        {f"{key}_pct": latency_delta(rows, key) for key in latency_keys}
    )
    config_ok = {
    ...
    for arm in ARMS:
        health_paths.extend(
            (root / arm / "health_pre.log", root / arm / "health_post.log")
        )
    checks = {
        "external_dual_card_lease_proven": "LEASE_CHECK PASS cards=0,1"
        in read(root / "lease_check.txt"),
        "checkpoint_audit_pass": load_json(root / "checkpoint_audit.json").get("status")
        == "PASS",
        "artifact_hashes_stable": artifact_before == artifact_after and len(hashes) >= 20,
        ...
        "all_w8a8_shims_installed": all(row["w8a8_installed"] for row in rows.values()),
        "all_replicated_mtp_markers_exact": all(
            row["replicate_markers"] for row in rows.values()
        ),
        ...
        "all_soaks_stable": all(
            0.95 <= row["soak_ratio"] <= 1.10 for row in rows.values()
        ),
    ...
    summary = {
        "claim": "c4_gdn_out_proj_int8_abba_qualification",
        "arms": rows,
    ...
    with (root / "summary.tsv").open("w", encoding="ascii") as handle:
        ...
        for arm in ARMS:
            handle.write(
                arm
                + "\t"
                + "\t".join(f"{rows[arm][column]:.4f}" for column in columns)
                + "\n"
            )
    lines = [f"VERDICT -> {'PASS' if passed else 'FAIL'}"]
    lines.extend(
        f"DELTA {key}={result:+.3f}%" for key, result in sorted(deltas.items())
    )
    lines.extend(
        f"CHECK {key}={'PASS' if result else 'FAIL'}"
