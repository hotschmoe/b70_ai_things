#!/usr/bin/env bash
# Candidate-only C4 mechanism gate for target GDN out_proj RTN-INT8.
#
...
ANALYZER="$REPO/sglang/analyze_c4_gdn_out_proj_int8_mechanism.py"
STAMP="${STAMP:-$(date -u +%Y%m%dT%H%M%SZ)}"
OUT="${OUT:-$REPO/results/logs/c4_gdn_out_proj_int8_mechanism_$STAMP}"
PROFILE_NAME="c4_gdn_out_proj_int8_mechanism_$STAMP"
PROFILE_DIR="$ROOT/sgl_cache/$PROFILE_NAME"
NAME="c4_gdn_out_proj_int8_mechanism"
PORT="31003"
SERVED="qwen36-27b-W8A8-sqgptq-GDN-OUT-RTN-mtp-c4-mechanism"
MODEL_CONTAINER="/models/qwen3.6-27b/w8a8-sqgptq-gdn-out-proj-int8"
MODEL_HOST="$REPO/models/files/qwen3.6-27b/w8a8-sqgptq-gdn-out-proj-int8"
...
artifacts=(
  ...
  "$PREPARE"
  "$ANALYZER"
  "$REPO/sglang/03_c4_gdn_out_proj_int8_mechanism.sh"
  ...
  "$MODEL_HOST/GDN_OUT_PROJ_INT8_NOTE.txt"
)
...
IMAGE_ID="$(docker image inspect sglang-xpu:mtp --format '{{.Id}}')"
{
  ...
  echo "claim=c4_gdn_out_proj_int8_mechanism"
