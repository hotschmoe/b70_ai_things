...
class AWQLinearKernel:
    ...
    def apply(
        ...
        # b70 XPU patch: dequant returns scales.dtype (fp16); cast to the act dtype so a
        # bf16 serve (validated GDN regime) matmuls bf16@bf16 instead of crashing on a mismatch.
        if out.dtype != reshaped_x.dtype:
            out = out.to(reshaped_x.dtype)
