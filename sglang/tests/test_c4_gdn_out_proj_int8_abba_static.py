#!/usr/bin/env python3
"""CPU-only contract tests for the GDN out_proj-only INT8 A-B-B-A campaign."""
...
def load_analyzer():
    path = SGLANG_DIR / "analyze_c4_gdn_out_proj_int8_abba.py"
    spec = importlib.util.spec_from_file_location("c4_gdn_out_proj_abba", path)
...
class RunnerContractTest(unittest.TestCase):
    def test_runner_preserves_promoted_and_disables_experimental_flags(self):
        script = (SGLANG_DIR / "04_c4_gdn_out_proj_int8_abba.sh").read_text(
            encoding="ascii"
        )
        for required in (
            ...
            "qwen36_w8a8_sqgptq_gdn_out_proj_rtn_quantization.json",
