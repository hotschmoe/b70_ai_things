...
SO = "/work/push_ar/libxpu_push_ar_graph.so"
...
def run(rank, world):
    ...
    with torch.xpu.stream(s):
        ...
        print(f"[r{rank}] recording ar_allreduce_graph...", flush=True)
        ...
        g.replay()
        torch.xpu.synchronize()
        # z was reset by capture side effects; value check: ((rank+1)+1) summed = 2+3=5, *2 = 10
        print(f"[r{rank}] replay ok z0={float(z[0])} (expect 10.0)", flush=True)
