#!/usr/bin/env bash
# Quantize Qwen3.8-27B (same Qwen3_5 hybrid as 3.6) -> compressed-tensors W8A8 INT8.
# Copy of scripts/49 with 3.8 defaults. Do NOT rewrite 49.
#
# GPU-ACCELERATED: llmcompressor SequentialPipeline keeps the ~52GB BF16 model on CPU and
# onloads one layer at a time onto B70 card 0 (ZE_AFFINITY_MASK=0). GPTQ Hessians / calib
# forwards run on XPU. SequentialPipeline cannot usefully split across two cards -- gpu-run
# still holds BOTH cards so nothing else starts. AutoRound 2xpu is scripts/65 if we A/B later.
#
# First 3.8 artifact: METHOD=gptq SMOOTHQUANT=0 (auto SmoothQuant ValueErrors on the 16/64
# full-attn split). SMOOTHQUANT=selective is the follow-up recovery lever (see
# docs/literature/07_w8a8_int8_recovery.md section 9).
#
# Env: SRC, OUT (full path) or OUTNAME (under REPO/models/files/qwen3.8-27b/), DEVICE=xpu|cpu,
#      METHOD=gptq|rtn, SAMPLES (512), SEQLEN (2048), SMOOTH (0.8), SMOOTHQUANT=0|selective|1,
#      IGNORE, DATAFREE=0, IMG.
set -uo pipefail
ROOT=/mnt/vm_8tb/b70
...
OUTNAME="${OUTNAME:-w8a8-${QMETH}}"
# Hybrid Qwen3_5: default SmoothQuant OFF (auto pairing throws). Override with selective.
case "$SCHEME" in *A16) SQD=0;; *) SQD=0;; esac
SMOOTHQUANT="${SMOOTHQUANT:-$SQD}"
# Same ignore list as 3.6: DeltaNet linear_attn + vision + MTP + lm_head stay BF16.
IGNORE="${IGNORE:-lm_head re:.*linear_attn.* re:.*visual.* re:.*mtp.*}"
OUT="${OUT:-$REPO/models/files/qwen3.8-27b/$OUTNAME}"
LOG="$ROOT/results/quant38_w8a8_$(date +%Y%m%d_%H%M%S).log"
mkdir -p "$ROOT/results" "$ROOT/pip_cache" "$(dirname "$OUT")"
[ -d "$SRC" ] || { echo "MISSING 3.8 BF16 at $SRC (models/files/qwen3.8-27b/bf16)"; exit 1; }
...
echo "=== 3.8 W8A8 quant: device=$DEVICE method=$METHOD sq=$SMOOTHQUANT samples=$SAMPLES out=$OUT ignore=[$IGNORE]  log=$LOG ==="

docker run --rm --name quant38 "${GPUARGS[@]}" --ipc=host --shm-size 32g \
  -v "$ROOT:$ROOT" -v "$REPO:$REPO" \
  -e HF_HOME="$ROOT/hf_cache" -e XDG_CACHE_HOME="$ROOT/vllm_cache" \
  -e OMP_NUM_THREADS=32 -e PIP_CACHE_DIR="$ROOT/pip_cache" \
  -e SRC="$SRC" -e OUT="$OUT" -e DEVICE="$DEVICE" -e METHOD="$METHOD" \
...
SQMODE=os.environ.get("SMOOTHQUANT","0").strip().lower()
