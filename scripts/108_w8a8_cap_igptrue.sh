#!/usr/bin/env bash
# Bug B probe: 27B W8A8 body, TP=2, PIECEWISE CAPTURE with use_inductor_graph_partition=TRUE, no-MTP,
# + the dummy-weight_scale shim (unblocks the IGP=true KeyError). Does IGP=true captured TP=2 generate coherently?
set -uo pipefail
ROOT=/mnt/vm_8tb/b70; CKPT=/models/Qwen3.6-27B-W8A8-sqgptq-mtp-graft; NAME=vllm_w8a8_igptrue; PORT=8000
...
CC="{\"cudagraph_mode\":\"PIECEWISE\",\"use_inductor_graph_partition\":true,\"cudagraph_capture_sizes\":[1,2,4,8],\"splitting_ops\":[$ATTN],$PASS}"
...
docker run -d --name "$NAME" --device /dev/dri -v /dev/dri/by-path:/dev/dri/by-path \
  ...
  -v "$ROOT/scripts/108_wscale_shim:/opt/wscale_shim:ro" \
  ...
  serve "$CKPT" --served-model-name w8a8igptrue --host 0.0.0.0 --port "$PORT" \
  ...
  --limit-mm-per-prompt '{"image":0,"video":0}' --compilation-config "$CC" >/dev/null
echo "launched $NAME (IGP=true captured no-MTP + dummy-wscale shim)"
