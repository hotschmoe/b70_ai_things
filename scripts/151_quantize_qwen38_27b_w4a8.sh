#!/usr/bin/env bash
# 151: Quantize Qwen3.8-27B -> compressed-tensors W4A8 with the GDN linear_attn projections ALSO
# quantized (int8). COPY of scripts/149 (do not rewrite 149). 3.8 is the same Qwen3_5 hybrid
# (64 layers, 16 full-attn, 48 GDN). Campaign: docs/20260820_qwen38_w4a8_campaign.md.
#
# First fire: DATAFREE=1 (RTN, accuracy-be-damned pipeline smoke). Second fire: DATAFREE=0
# METHOD=gptq SMOOTHQUANT=selective (the calibrated artifact).
#
# 3.6 w4a8-sqgptq ignored ALL 432 linear_attn.* tensors -> 10.3 GiB of GDN stayed bf16 ->
# NO single-card KV headroom. 151 keeps that surgical GDN-INT8 group.
#
...
#
# Env: SRC (bf16 source), OUT, CARD (XPU card for calib onload, DEFAULT 0 -- pair with --card 1 kernels),
...
# Run:  cd /mnt/vm_8tb/github/b70_ai_things && B70_GPU_LOCK_TIMEOUT=0 ./bin/gpu-run --card 0 \
#         env DATAFREE=1 bash scripts/151_quantize_qwen38_27b_w4a8.sh
# (GPU lease REQUIRED for stage A. Stage B is CPU-only. 3.8 needs int8g-v0260, not v0230.)
set -uo pipefail
ROOT=/mnt/vm_8tb/b70
REPO=/mnt/vm_8tb/github/b70_ai_things
IMG="${IMG:-vllm-xpu-env:int8g-v0260}"
SRC="${SRC:-$REPO/models/files/qwen3.8-27b/bf16}"
CARD="${CARD:-0}"; DEVICE="${DEVICE:-xpu}"; METHOD="${METHOD:-gptq}"
SAMPLES="${SAMPLES:-128}"; SEQLEN="${SEQLEN:-2048}"; SMOOTH="${SMOOTH:-0.8}"
SMOOTHQUANT="${SMOOTHQUANT:-selective}"; DATAFREE="${DATAFREE:-1}"
# Tag the output so RTN and GPTQ NEVER collide (same rule as scripts/150).
QMETH="$METHOD"; [ "$DATAFREE" = 1 ] && QMETH="rtn"
OUT="${OUT:-$REPO/models/files/qwen3.8-27b/w4a8-${QMETH}-gdn}"
RAW="${RAW:-$OUT-raw}"
SIBLING="${SIBLING:-$REPO/models/files/qwen3.8-27b/w8a8-gptq}"
...
LOG="$REPO/results/logs/151_qwen38_w4a8_$(date +%Y%m%d_%H%M%S).log"
...
echo "=== 151 Qwen3.8 W4A8+gdnint8 quant: device=$DEVICE card=$CARD method=$METHOD samples=$SAMPLES datafree=$DATAFREE" | tee "$LOG"
...
docker rm -f q151_quant 2>/dev/null || true
docker run --rm --name q151_quant "${GPUARGS[@]}" --ipc=host --shm-size 32g \
...
print("DONE_151_STAGE_A", OUT, flush=True)
PY
  ' 2>&1 | tee -a "$LOG"
RC=${PIPESTATUS[0]}
[ "$RC" = 0 ] || { echo "=== stage A FAILED rc=$RC (log $LOG) ==="; exit "$RC"; }
grep -q "DONE_151_STAGE_A" "$LOG" || { echo "=== stage A did not complete (no DONE marker) ==="; exit 1; }
fi

# ---------------------------------------------------------------------------------------------------------
# Stage B -- CPU-only prepack + graft-check + shard split + metadata verify. NO GPU, NO lease needed.
# ---------------------------------------------------------------------------------------------------------
docker rm -f q151_pack 2>/dev/null || true
docker run --rm --name q151_pack -v "$REPO:$REPO" -v "$ROOT:$ROOT" \
...
with open(os.path.join(OUT,"GDN_INT8_NOTE.txt"),"w") as f:
    f.write(
        "qwen3.8-27b W4A8 (int4 group-128 + dynamic int8 act) with the 3 big GDN\n"
        ...
        "same on-disk scheme as the 3.6 w8a8-sqgptq-gdnint8 precedent, GPTQ or RTN per DATAFREE,\n"
        "with CORRECT compressed-tensors metadata: config_groups declares an int8-channel group\n"
        "targeting the projections and the ignore list is surgical (only lm_head, visual, mtp,\n"
        "linear_attn.in_proj_b, linear_attn.in_proj_a stay bf16; conv1d/A_log/dt_bias/norm are\n"
        "not Linear). vLLM consumes the GDN int8 natively via CompressedTensorsW8A8Int8 ->\n"
        "XPUInt8ScaledMMLinearKernel; the fused in_proj_qkvz (int8+int8) and in_proj_ba\n"
        "(bf16+bf16) stay scheme-homogeneous.\n"
        "int4 weights are PREPACKED int32 [out,in/8] (is_prepacked_w4a8=true); the int8 GDN\n"
        "weights are NOT packed. Produced by scripts/151_quantize_qwen38_27b_w4a8.sh.\n"
        "GPU-gated acceptance still required. Campaign:\n"
        "docs/20260820_qwen38_w4a8_campaign.md. Accuracy is a later loop.\n")
print("DONE_151_STAGE_B", OUT, flush=True)
PY
  ' 2>&1 | tee -a "$LOG"
RC=${PIPESTATUS[0]}
[ "$RC" = 0 ] || { echo "=== stage B FAILED rc=$RC (log $LOG) ==="; exit "$RC"; }
grep -q "DONE_151_STAGE_B" "$LOG" || { echo "=== stage B did not complete ==="; exit 1; }

[ "$KEEP_RAW" = 1 ] || { echo "=== removing RAW intermediate $RAW (KEEP_RAW=1 to keep) ==="; rm -rf "$RAW"; }
echo "=== 151 DONE -> $OUT ==="; du -sh "$OUT" 2>/dev/null
