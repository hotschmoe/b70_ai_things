...
curl -fsS "${AUTH[@]}" "$BASE/v1/completions" -d "{\"model\":\"$MODEL\",\"prompt\":\"hello\",\"max_tokens\":8,\"temperature\":0}" >/dev/null
...
if [ "$MODE" = prefill ]; then
  ...
  curl -fsS "${AUTH[@]}" "$BASE/v1/completions" -d "$BODY" | python3 -c "import sys,json; d=json.load(sys.stdin); print('  prompt_tokens=',d['usage']['prompt_tokens'],'completion=',d['usage']['completion_tokens'])"
else
  ...
  curl -fsS "${AUTH[@]}" "$BASE/v1/completions" -d "{\"model\":\"$MODEL\",\"prompt\":\"Write a long detailed essay on computing history.\",\"max_tokens\":256,\"temperature\":0,\"ignore_eos\":true}" | python3 -c "import sys,json; d=json.load(sys.stdin); print('  completion_tokens=',d['usage']['completion_tokens'])"
fi
