...
| 9 | TP=2 decode on the dense hybrid is ALL-REDUCE-BOUND (43% of device time; GEMM 39% is at roofline), and that all-reduce is the MTP spec-decode `vllm::all_gather` realized as an eager oneCCL SUM over PCIe (no Battlemage P2P) that BYPASSES the push-AR (which patches only XpuCommunicator.all_reduce). Moving it to the eager host-barrier push-AR is 2.4x SLOWER (CPU-spin barrier per call x ~631 gathers/step >> device saving); only the GRAPH-recorded push-AR (do_ar) is fast, so the real fix is CAPTURING the gather. | nvfp4/w8a8 / vLLM 0.25.1 (27b, TP=2) | any TP>1 spec-decode on XPU without P2P; the push-AR all_reduce override does NOT cover gather-internal all-reduces | traced (research/profiling/parse_trace.py). ALL cheap transports measured + ruled out: host-barrier redirect 2.4x slower; eager-async 1-host-sync redirect 2.5x slower (48.9->19.8, zero-host handoff PROVEN impossible on B70 -- broken get_native_queue / EU-spin hangs); oneCCL (0 host sync, async device) = baseline; graph push-AR (do_ar, 0 host sync) = fast but CAPTURE-ONLY; PP=2 = no SupportsPP; torch2.13/#2992 = captured oneCCL node ~85us > push-AR ~40us (NOT faster) + ABI-break rebuild. ONLY remaining fix = capture the MTP all_gather (fast do_ar reaches it) or cut the gather count -- both hard/deferred. JOURNAL 2026-07-21 s2+s3. |
| 10 | Native int4 s4xs4 -> s32 DPAS on B70 is REAL + int-exact (emits dpas.s4.s4.8.8, relerr 3.5e-8), but a naive tiled ESIMD mainloop caps ~64 TOPS (M512==M2048, mainloop-bound) vs int8 367 / bf16 183 -- realizing the 2x-int8 int4 ceiling needs an XeTLA-grade mainloop. W4A4 accuracy needs a Hadamard rotation MANDATORY (no-rot cosine 0.796 -> +block-Had 0.973 -> +group128 0.988 ~W4A16, still < W8A8 0.999) + an online FWHT kernel we lack. | int4 weights+acts / ESIMD (27b prefill) | any W4A4/W4A16 kernel; the int4-DPAS atom is a showcase, not yet a serve path | s4s4_gemm int-exact + 64 TOPS (vllm/nvfp4/proto_int4/w4a4/); W4A4 = showcase NOT serve (decode is BW-bound so prefill TOPS don't translate; W4A8 stays the real 4-bit serve path). JOURNAL 2026-07-21 s3. |
