#!/usr/bin/env bash
# zml/run_w8a8_mtp_measure_tp2_gpu.sh -- Step 1: NEXTN/MTP draft ACCEPT-RATE measurement on the W8A8
...
#   cd /mnt/vm_8tb/github/b70_ai_things && ./bin/gpu-run bash zml/run_w8a8_mtp_measure_tp2_gpu.sh
set -uo pipefail
...
export ZML_MTP_MEASURE=1                                  # <-- MTP observer/accept-rate mode
echo "=== zml W8A8 27B TP=2 MTP-MEASURE  model=$MODEL seqlen=$SEQLEN topk=$TOPK  $(date) ==="
set +e
