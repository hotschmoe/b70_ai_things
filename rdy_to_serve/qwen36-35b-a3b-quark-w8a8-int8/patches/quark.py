...
class QuarkW8A8Int8DequantXPU(QuarkScheme):
    ...
    vLLM 0.23 already DISPATCHES our dynamic-per-token int8 checkpoint to QuarkW8A8Int8,
    but that scheme calls choose_scaled_mm_linear_kernel, whose registry has NO XPU entry
    (KeyError: PlatformEnum.XPU). XPU also has the working Triton fused-MoE path, so the
    256 routed experts stay true int8; only the minority linear layers (linear_attn.*,
    mlp.shared_expert.*) need this fallback: dequant the per-channel symmetric int8 weights
    to the model dtype once at load and run a plain GEMM (effectively W8A16 -- activations
    not quantized -> MORE accurate than the checkpoint's W8A8, correctness-first).
    """
