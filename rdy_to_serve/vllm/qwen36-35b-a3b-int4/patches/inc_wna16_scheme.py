...
class INCWna16Scheme(INCScheme):
    ...
    def get_linear_method(
        ...
        if current_platform.is_xpu():
            if layer_config.bits == 4 and layer_config.sym:
                ...
                # b70 patch: ARK (auto_round_kernel.woqgemm, a ctypes call) is NOT
                # dynamo-traceable -> hard graph-break crash under torch.compile /
                # PIECEWISE capture (our headline config). Default to the
                # INCXPULinearMethod fallback (torch.ops._xpu_C.int4_gemm_w4a16, a
                # real torch op that traces + captures). B70_INC_ARK=1 re-enables
                # ARK for eager-only experiments.
                import os

                if os.environ.get("B70_INC_ARK", "0") == "1":
    ...
    def get_moe_method(
        ...
        del config, prefix
        # b70 patch (vllm_moe_xpu, re-ported from the v0.23 inc.py patch; see
        # vllm/contrib/vllm_moe_xpu/README.md): upstream returns Unquantized for
        # MoE on XPU, which holds the 256 int4 experts in bf16 (~70 GB) and OOMs
        # a 32 GB B70 at weight load. The pure-Triton MoeWNA16 fused-moe path
        # (fused_moe_kernel_gptq_awq) runs fine on XPU -- should_moe_wna16_use_cuda
        # is False there, so no CUDA/Marlin op is reached. Route gptq/awq-packed
        # int experts to it directly, skipping the CUDA-only Marlin capability
        # probes in _resolve_gptq_moe/_resolve_awq_moe.
        if current_platform.is_xpu():
            if layer_config.bits in (4, 8) and (
                layer_config.is_gptq or layer_config.is_awq
            ):
