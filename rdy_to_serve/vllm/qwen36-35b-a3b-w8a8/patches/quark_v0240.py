...
# b70 patch (re-ported 2026-07-04 from the proven v0.23 patch; see ./README.md):
# parameter classes + helper for the int8 dequant linear scheme below.
# replace_parameter moved to utils.layer_utils in v0.24.0.
...
class QuarkW8A8Int8DequantXPU(QuarkScheme):
    ...
    vLLM (0.23 and 0.24 alike) DISPATCHES our dynamic-per-token int8 checkpoint
    to QuarkW8A8Int8, but that scheme resolves an int8 scaled-mm kernel from
    _POSSIBLE_INT8_KERNELS, which has NO XPU entry (KeyError: PlatformEnum.XPU).
    XPU has the working Triton fused-MoE path, so the 256 routed experts stay
    true int8; only the minority linear layers (linear_attn.*,
    mlp.shared_expert.*) need this fallback: dequant the per-channel symmetric
    int8 weights to the model dtype once at load and run a plain GEMM
    (effectively W8A16 -- activations not quantized -> MORE accurate than the
    checkpoint's W8A8, correctness-first). Opt out with B70_INT8_LINEAR=native
    (falls through to stock QuarkW8A8Int8; needs the :int8g-v0240 image + the
    runtime-mounted int8 _xpu_C .so).
    """
    ...
    def process_weights_after_loading(self, layer: torch.nn.Module) -> None:
        ...
        # Compute OUTSIDE inference_mode so w_deq tracks a version counter (else
        # torch.compile functionalization trips on the inference tensor).
        with torch.inference_mode(False), torch.no_grad():
            ws = layer.weight_scale.to(torch.float32).reshape(-1, 1)
...
class QuarkConfig(QuantizationConfig):
    ...
    def _get_scheme_from_config(
        ...
        if self._is_nvfp4(weight_config, input_config):
            ...
            weight_qscheme = cast(str, weight_config.get("qscheme"))
            # b70: XPU default = the bf16-dequant (W8A16) linear scheme; stock
            # QuarkW8A8Int8 KeyErrors on XPU (no _POSSIBLE_INT8_KERNELS entry).
            # B70_INT8_LINEAR=native opts INTO stock dispatch (requires the
            # :int8g-v0240 image, whose bake registers the oneDNN
            # XPUInt8ScaledMMLinearKernel + the runtime-mounted _xpu_C .so).
            if current_platform.is_xpu():
                ...
                logger.info_once(
                    "b70: B70_INT8_LINEAR=%s -> stock QuarkW8A8Int8 dispatch",
                    os.environ["B70_INT8_LINEAR"],
                )
            return QuarkW8A8Int8(
                qscheme=weight_qscheme,
