...
case "$MOE_BACKEND" in
  auto) moe_backend_suffix="" ;;
  xpu) moe_backend_suffix="_moe_xpu" ;;
  triton) moe_backend_suffix="_moe_triton_intervention" ;;
  *) echo "MOE_BACKEND must be auto, xpu, or triton" >&2; exit 2 ;;
esac
[ "$MOE_BACKEND" != triton ] || [ "$SOURCE_STACK" = june-e190 ] || {
  echo "MOE_BACKEND=triton intervention requires SOURCE_STACK=june-e190" >&2
  exit 2
}
TRITON_INTERVENTION_VALUE=0
[ "$MOE_BACKEND" != triton ] || TRITON_INTERVENTION_VALUE=1
...
echo "config -> TP=2 PP=1 graph=$CGMODE attention=${ATTN:-default} moe_backend=$MOE_BACKEND triton_int8_intervention=$TRITON_INTERVENTION_VALUE splitops=default igp=false async=1 mtp=0 prefix_cache=0 maxlen=32768 maxseqs=24 util=0.90"
if [ "$SOURCE_STACK" = june-e190 ]; then
...
if [ "$MOE_BACKEND" = triton ]; then
  required_markers=(
    "${required_markers[@]/Using XPU Int8 MoE backend/Using TRITON Int8 MoE backend}"
