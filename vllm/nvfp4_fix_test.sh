...
NAME="${NAME:-nvfp4_repro}"; PORT="${PORT:-8078}"; CARD="${CARD:-0}"
...
echo "===== [NVFP4 $CFG_LABEL] B70_EXTRA_ENV='${B70_EXTRA_ENV:-}' TP=1 card=$CARD soak_ceil=$CEIL_TOK/$CEIL_SEC $(date -u +%FT%TZ) =====" | tee -a "$RESULTS"
docker rm -f "$NAME" >/dev/null 2>&1

# Launch single-card NVFP4 crashing config (bf16 KV to isolate from the repetition fault).
NAME="$NAME" PORT="$PORT" CARD="$CARD" TP=1 MODE=fused GRAPH=1 MTPTOK=5 KV_FP8=0 \
  CAPSIZES=1,2,4,8 MAXLEN=8192 UTIL=0.85 B70_DEBUG=1 \
