#!/usr/bin/env bash
# F09e: static MTP8 FULL graph at 262K with matched c2/c4 throughput probes.
set -euo pipefail
...
exec env \
  STAMP="$STAMP" \
  RESULT_DIR="${RESULT_DIR:-$ROOT/results/f09e_qwen38_fp8_mtp8_c2c4/$STAMP}" \
  CACHE_DIR="${CACHE_DIR:-$ROOT/cache/f09e_qwen38_fp8_mtp8_c2c4/$STAMP}" \
  NAME="${NAME:-qwen38-fp8-f09e-mtp8-c2c4-$STAMP}" \
  PORT="${PORT:-18202}" \
  ...
  CAMPAIGN_LABEL=F09e \
  SERVED=qwen3.8-27b-FP8-official-W8A16-mtp8-p2p1-fp16kv-f09e-full-c4-262k \
