#!/usr/bin/env python3
"""Validate two F08b MTP8 lifetimes and emit qualification evidence."""
...
def main() -> int:
    ...
    full_medians = [
        item["summary"]["tok_s_after_ttft_full"]["median"]
        for item in performance
    ]
    for path, item, rate in zip(attempts, performance, primary):
        ...
        require_log(path / "verdict.txt", "VERDICT -> F08b PASS")
    ...
    output = {
        "schema": "b70.qwen38-fp8-f08b-qualification.v1",
        "verdict": "qualified_local_full_graph_mtp8_route",
        "primary_tok_s_attempts": primary,
        "primary_tok_s_mean": statistics.mean(primary),
        "primary_tok_s_min": min(primary),
        "full_post_ttft_tok_s_medians": full_medians,
        ...
        "publisher_mtp8_exact_prompts": publisher_counts,
        "publisher_mtp8_identity_exact": all(count == 12 for count in publisher_counts),
        "performance_sha256": [
