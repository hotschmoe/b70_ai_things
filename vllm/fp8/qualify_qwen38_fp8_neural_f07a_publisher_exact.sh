#!/usr/bin/env bash
# F07a: exact r32 single-stream compiler and service-shape reproduction.
set -euo pipefail
...
exec env \
  ...
  CAMPAIGN_ID=f07a \
  CAMPAIGN_LABEL=F07a \
  CONTAINER_PREFIX=qwen38-fp8-neural-f07a-publisher-exact \
  ANALYZER_SCHEMA=b70.qwen38-fp8-neural-f07a-publisher-exact.v1 \
  COMPLETION_ROUTE=mtp1-packed-rms-publisher-compile-p2p1 \
  COMPILE_ORACLE=0 \
  SHARED_CACHE=0 \
  REQUIRE_REFERENCE_EXACT=1 \
  SERVED=qwen3.8-27b-FP8-official-W8A16-mtp1-p2p1-fp16kv-f07a-r32-exact \
  PUBLISHER_A="$PUBLISHER_A" \
  PUBLISHER_B="$PUBLISHER_B" \
  STAMP="$STAMP" \
  RESULT_DIR="${RESULT_DIR:-$ROOT/results/f07a_qwen38_fp8_neural_publisher_exact/$STAMP}" \
  CACHE_ROOT="${CACHE_ROOT:-$ROOT/cache/f07a_qwen38_fp8_neural_publisher_exact/$STAMP}" \
