#!/usr/bin/env bash
# F09c: bounded V2-runner attempt to retain FULL graph with dynamic MTP depth.
set -euo pipefail
...
exec env \
  STAMP="$STAMP" \
  RESULT_DIR="${RESULT_DIR:-$ROOT/results/f09c_qwen38_fp8_v2_dynamic_full/$STAMP}" \
  CACHE_DIR="${CACHE_DIR:-$ROOT/cache/f09c_qwen38_fp8_v2_dynamic_full/$STAMP}" \
  NAME="${NAME:-qwen38-fp8-f09c-v2-dynamic-full-$STAMP}" \
  PORT="${PORT:-18200}" \
  ...
  CAMPAIGN_LABEL=F09c \
  SERVED=qwen3.8-27b-FP8-official-W8A16-dynamic-mtp8-mtp1-v2-p2p1-fp16kv-f09c \
