#!/usr/bin/env python3
"""B70 nightly port of patch_mtp_bf16_draft.py (public vLLM XPU nightlies).
...
pattern. GPTQ-preserved checkpoints often have dynamic=null, so we extend
the condition with an env gate: B70_MTP_BF16_DRAFT=1.

Source: SergiioB/intel-arc-pro-b70-inference-cookbook (MIT).
For our baked vllm-xpu-env:v0260 (0.26.0), use patch_mtp_bf16_draft_v0260.py
instead -- the anchors differ.
"""
import os
import sys
...
def main() -> None:
    ...
    if old not in t:
        # Fall through to v0260 patch path if caller mounted both.
        sys.exit(
            "anchor not found -- nightly qwen3_5_mtp.py changed; "
            "try patch_mtp_bf16_draft_v0260.py for 0.26.0"
        )
