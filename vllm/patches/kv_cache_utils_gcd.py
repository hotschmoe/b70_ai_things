...
def _get_kv_cache_groups_uniform_page_size(
    ...
    # [B70 PATCH -- DFlash KV eater] Pick group_size to MINIMIZE total padding
    # layers, searching group_size in [min_num_layers, max_num_layers].
    #
    # Upstream defaults group_size to min_num_layers (with a 1.5x max override
    # for near-equal counts). That default is poisoned by a small spec-decode
    # drafter: DFlash adds a 5-layer attention type to this hybrid (16 full +
    # 48 mamba), so group_size collapses to 5 and the 16 target full-attn
    # layers pad to 20 (25% KV waste) plus mamba 48 -> 50 (4.17%) = 6 wasted
    # layers -> ~21% of per-token KV. Searching for the min-padding group_size
    # instead finds 8 here: target 16 and mamba 48 both divide evenly (0 pad),
    # only the drafter pads 5 -> 8 (3 layers), halving the waste.
    #
    # The lower bound of min_num_layers is REQUIRED: group_size must be >= the
    # smallest type's layer count so that type stays in ONE group. vLLM asserts
    # "All drafting layers should belong to the same kv cache group" and the
    # drafter is the smallest type, so group_size < 5 (e.g. gcd=1) crashes the
    # worker. Staying >= min guarantees every type keeps <= the count it had at
    # group_size=min, and the drafter (the min type) stays whole.
    #
    # Non-drafter cases are UNCHANGED: MTP {16 full, 48 mamba} -> 16 (0 pad,
    # == old min); eagle {12 sw, 13 full} -> 13 (== the old 1.5x override).
    _b70_counts = [len(layers) for layers in same_type_layers.values()]
    _b70_best = None
    for _b70_gs in range(min_num_layers, max_num_layers + 1):
        _b70_pad = sum((_b70_gs - (c % _b70_gs)) % _b70_gs for c in _b70_counts)
        _b70_ngroups = sum(cdiv(c, _b70_gs) for c in _b70_counts)
        # minimize padding first, then group count (fewer groups = larger gs)
        _b70_key = (_b70_pad, _b70_ngroups)
        if _b70_best is None or _b70_key < _b70_best:
            _b70_best = _b70_key
            group_size = _b70_gs
    grouped_layers = []
