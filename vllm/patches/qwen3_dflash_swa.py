...
class DFlashQwen3Attention(nn.Module):
    ...
    def __init__(
        ...
        self.attn = Attention(
            ...
            attn_type=attn_type,
            # [B70 PATCH -- DFlash KV eater] Honor the drafter's sliding window.
            # Applied UNIFORMLY to all 5 layers (the config is 4 sliding + 1
            # full, but the DFlash context-KV path requires all drafter layers
            # in ONE kv cache group, so they must share ONE spec type -> all
            # sliding). Caps the drafter's per-token KV at the 2048 window
            # instead of the full max_model_len (~10 KB/tok -> windowed).
...
class DFlashQwen3DecoderLayer(nn.Module):
    def __init__(
        ...
        *,
        ...
        # [B70 PATCH -- DFlash KV eater] Uniform sliding window for all drafter
        # layers (env B70_DFLASH_SWA=1). config declares 4 sliding + 1 full,
        # but all drafter layers must share ONE kv cache group -> apply the same
        # 2048 window to every layer. Off by default (=None -> full attention,
        # upstream behaviour) so this file is inert unless B70_DFLASH_SWA=1.
        import os as _os
        _swa = None
        if _os.environ.get("B70_DFLASH_SWA", "0") == "1" and getattr(
            config, "use_sliding_window", False
        ):
            _swa = getattr(config, "sliding_window", None)
