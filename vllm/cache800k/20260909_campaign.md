...
## Full-feature endpoint handed to user workloads

CONFIG -> Trial 20260909T220656Z, calibrated FP8 E4M3 KV, INT4 weights,
MTP3, FULL_DECODE_ONLY XPU graphs, prefix caching, 200000 max context,
four sequence slots, 860000 logical KV tokens, no RAM offload.
COMMAND -> hotschmoe-dd-fp8-trial.service with the matched prior workload
evidence drop-in; fresh compiler cache, short quality and 32K A/B/A reuse.
RESULT -> Pre-health and both collective controls pass; graph capture
completes. Short quality24/24 exact. Prefix reuse3/3 exact, 30400 hit tokens,
TTFT16.0767/15.4517/1.1847s, zero preemptions and242 accepted MTP tokens.
PASSED.json written; service active. Authenticated public /health and
/v1/models return200 at http://192.168.10.5:18080. Canonical served ID matches
evals/configs/models.yaml; alias hotschmoe-dd and existing key preserved.
VERDICT -> Ready for the requested real-workload trial with all requested
features enabled. Prior four196K retrievals passed; four sustained8K output
reserves remain untested and cold concurrent prefill is slow. Long guides
were coherent but not byte-identical. Not promoted to the verified shelf.
Broad campaign and recurring review timer remain paused. Live serving owns
both GPU leases; final teardown/post-health will occur when it is stopped.

## Reboot service preparation

CONFIG -> Existing system hotschmoe-dd.service is enabled but points to FP16;
the live FP8 user service is static and is not enabled at boot.
COMMAND -> Prepare cache800k/hotschmoe-dd.service; systemd-analyze verify.
RESULT -> Replacement unit validates, retaining Docker/mount ordering,
hotschmoe ownership, GPU lease lifecycle, all FP8 serving features, and the
matched prior-evidence path used by the live service. Startup reruns quality
and prefix reuse. Existing live user service remains active and untouched.
VERDICT -> Ready for user sudo installation as the system hotschmoe-dd.service,
daemon-reload and enable, without --now or restart. Reboot execution remains
unverified. Keep the existing static user unit unenabled to avoid two boot
launchers. The referenced calibration and prior result directories must remain.

## Permanent public model name correction

CONFIG -> Client name must always be hotschmoe-dd. Previous FP8 launch put
the detailed physical-model ID first and hotschmoe-dd second.
COMMAND -> Change launcher primary name to hotschmoe-dd, retain physical ID
as secondary alias, and run validation using the client name. Record the
permanent-name contract in AGENTS.md and refresh the eval alias entry.
RESULT -> Six CPU tests pass, including rejection of physical-ID changes
while permitting alias reordering in matched prior-evidence validation.
VERDICT -> Prepared; requires service restart to change the running backend
name order. Model, scales, graph/MTP/prefix flags and endpoint URL unchanged.
