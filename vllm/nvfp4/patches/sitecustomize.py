...
# ---- (7) FUSED per-expert NVFP4 MoE (Track 11f / LOOP 61 capture-safe) -----------------------------
# Decode GRAPH cannot host-sync (LOOP 60: torch.unique().tolist() -> event.wait on a command graph).
# Implementation lives in fused_moe_apply.py (same PYTHONPATH as this file). Slot-static path for
# T<=SLOT_MAX; grouped unique() only for large eager prefill.
if os.environ.get("NVFP4_MOE_FUSED", "0") == "1":
    try:
        ...
        from fused_moe_apply import install as _install_fused_moe

        _install_fused_moe(_FMoE)
    except Exception as e:
