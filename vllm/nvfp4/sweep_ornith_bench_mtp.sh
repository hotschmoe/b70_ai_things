#!/usr/bin/env bash
# L60 follow-on: fused eager decode bench + optional MTP. GRAPH parked
# (fused MoE apply does torch.unique().tolist() -> XPU graph event.wait).
set -uo pipefail
REPO=/mnt/vm_8tb/github/b70_ai_things
SERVE="$REPO/vllm/nvfp4/serve_nvfp4_moe_35b.sh"
PROBE="$REPO/vllm/nvfp4/g1_probe.py"
STAMP=$(date -u +%Y%m%dT%H%M%SZ)
OUTDIR="$REPO/results/logs/ornith_nvfp4_bench_${STAMP}"
mkdir -p "$OUTDIR"
HOST=192.168.10.5
PORT=18080
...
export MODE=fused GRAPH=0 TP=1 CARD=0
export KV_FP8=0 LANGONLY=1 MAXSEQS=4 MAXLEN=8192 UTIL=0.90
export CAPSIZES= MTPTOK=
...
run_arm() {
  local arm="$1"
  export SERVED="ornith-1.5-35b-A3B-NVFP4-fused-${arm}"
  log "===== $arm MODE=fused GRAPH=0 MTPTOK=${MTPTOK:-off} ====="
