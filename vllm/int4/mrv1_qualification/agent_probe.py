#!/usr/bin/env python3
"""Frozen MRV1 qualification fork: explicit sampling and shared round namespace."""
import argparse
...
def main():
    ...
    p.add_argument('--cache-namespace', required=True, help='Shared within one round; unique between rounds')
    ...
    if not args.shared_cache or not args.cache_namespace or args.temperature not in (0, .7):
        p.error('qualification requires shared-cache, nonempty namespace, temperature 0 or .7')
    if args.bang_retries:
        p.error('qualification forbids recovery retries')
    ...
    def session(index):
                ...
                for attempt in range(args.bang_retries + 1):
                    ...
                    try:
                        if isolation_active:
                            ...
                            payload['cache_salt'] = args.cache_namespace
        ...
        for turn in range(args.turns):
            ...
            payload = {'model': args.model, 'messages': history, 'temperature': args.temperature, 'seed': args.seed, 'max_tokens': 512,
