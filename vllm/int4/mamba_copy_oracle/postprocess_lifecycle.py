#!/usr/bin/env python3
"""Frozen Mamba POSTprocess byte oracle lifecycle; selected inherited lease required."""
import argparse
import hashlib
import json
import os
...
def parse_oracle(path):
    ...
    for index, char in enumerate(text):
        ...
        try:
            ...
            continue
        if isinstance(value, dict) and value.get('execution') == 'actual installed run_fused_postprocess -> postprocess_mamba_fused_kernel':
            return value
...
def execute(plan):
    ...
    result = dict(card=card, image=plan['image'],
                  scope='Actual Mamba postprocess copy/reset only; no native GDN/model qualification')
    ...
    try:
        ...
        assert parsed['tp_size'] == plan['tp_size'] and parsed['kv_layout'] == plan['kv_layout']
        expected_pass = all(row['mismatched_bytes'] == 0 and row['untargeted_bytes_untouched']
                            and row['accepted_correct'] and row['accepted_input_untouched']
                            and row['accepted_buffers_distinct'] for row in parsed['cases'])
        assert parsed['passed'] == expected_pass
        assert all(row['passed'] == (row['mismatched_bytes'] == 0 and row['untargeted_bytes_untouched']
                    and row['accepted_correct'] and row['accepted_input_untouched'])
                   for row in parsed['cases'])
        assert all(row['accepted_result'] == [row['case']['accepted_out'], -777, -777, -777]
                   for row in parsed['cases'] if row['accepted_correct'])
