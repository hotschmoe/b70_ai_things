...
RESULT -> Five CPU tests pass, including100K/200K config/primary alias/P2P0, no fake qualification claim, tiny-only startup and missing prerequisite refusal. No GPU/install/start by this preparation. User selected200K is explicit in the drop-in and default;100K is only an explicit fallback option.
...
Readiness follows the actual bin/gpu-run parent relationship: systemd MAINPID is the lease shell and the Python owner is its direct child. READY records both; readiness verifies the live PPid, main PID pidfd and frontdoor socket ownership. An actual CPU-only bin/gpu-run test with temporary lock files proves this relationship without touching GPU devices. Final day-trial check --context200k passed after the prior lifecycle/post-health completed.
