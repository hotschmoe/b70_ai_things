...
GRAPH_ENV=(); GRAPH_DOCKER=(); EAGER=(--enforce-eager); CC=()
if [ "$GRAPH" = 1 ]; then
...
echo "=== serve W4A8 GRAPH=$GRAPH (eager=$([ "$GRAPH" = 1 ] && echo no || echo yes)) IMG=$IMG dtype=$DTYPE MAXLEN=$MAXLEN SEQS=$MAXSEQS ==="
echo "vllm ${ARGS[*]}"
docker run -d --name "$NAME" --device /dev/dri -v /dev/dri/by-path:/dev/dri/by-path \
  ...
  "${GRAPH_ENV[@]}" --entrypoint vllm "$IMG" "${ARGS[@]}"
